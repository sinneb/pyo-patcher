...
RED.nodes = (function() {
    ...
    function addLink(l) {
      console.log("in addlink function");
      console.log("before add links:");
        ...
        if (n._def.category != "config") {
            ...
            for (var j=0;j<wires.length;j++) {
                ...
                if (w.target.type != "subflow") {
                    ...
                    node.wires[w.sourcePort].push(w.targetPort);
                    node.wires[w.sourcePort].push(w.target._def.inputLabels[w.targetPort])
    ...
    function importNodes(newNodesObj,createNewIds,createMissingWorkspace) {
        ...
        for (i=0;i<new_nodes.length;i++) {
            ...
            if (n.wires) {
              console.log(n.wires);
                for (var w1=0;w1<(n.wires.length/3);w1++) {
                    var wires = (n.wires[w1] instanceof Array)?n.wires[w1]:[n.wires[w1]];
                    //console.log(wires.length);
                    for (var w2=0;w2<wires.length;w2=w2+3) {
                      //console.log(w2);
                        //if (node_map.hasOwnProperty(wires[w2])) {
                            //if (n.z === node_map[wires[w2]].z) {
                                var link = {source:n,sourcePort:w1,target:node_map[wires[w2]],targetPort:parseInt(wires[w2+1])};
                                console.log("adding link in import");
                                //console.log((wires[w2][1]));
...
RED.deploy = (function() {
      ...
      console.log("init");
        ...
        if (type == "default") {
            $('<li><span class="deploy-button-group button-group">'+
              '<a id="btn-deploy" class="deploy-button" href="#">'+
    ...
    function save(skipValidation,force) {
      ...
      var data = "text/json;charset=utf-8," + encodeURIComponent(flow);
                  ...
                  //console.log(portIndex);
                    ...
                    if (!existingLink) {
                      //console.log("adding link");
                        var link = {source:src,target:dst,sourcePort: src_port, targetPort: dst_port};
                        RED.nodes.addLink(link);
                    ...
                    if (d.dirty) {
                        ...
                        if (mouse_mode != RED.state.MOVING_ACTIVE) {
                            ...
                            if (d.inputs === 0 && !inputPorts.empty()) {
                                inputPorts.remove();
                                //nodeLabel.attr("x",30);
                            } else // if (inputPorts.empty()) {
//                               for (ii=0;ii<d.inputs;ii++) {
//                                 var inputGroup = thisNode.append("g").attr("class","port_input");
//                                 inputGroup.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10).attr("i",ii)
//                                     .on("mousedown",function(d){portMouseDown(d,1,ii);})
//                                     .on("touchstart",function(d){portMouseDown(d,1,ii);})
//                                     .on("mouseup",function(d){portMouseUp(d,1,ii);})
//                                     .on("touchend",function(d){portMouseUp(d,1,ii);})
...
//
//                             }// 
                              if (d.inputs === 1 && inputPorts.empty()) {
                                var inputGroup = thisNode.append("g").attr("class","port_input");
                            ...
                            } else if (d.inputs === 2 && inputPorts.empty()) {
                                var inputGroup = thisNode.append("g").attr("class","port_input");
                                inputGroup.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                                    .on("mousedown",function(d){portMouseDown(d,1,0);})
                                    .on("touchstart",function(d){portMouseDown(d,1,0);})
                            ...
                            } else if (d.inputs === 3 && inputPorts.empty()) {
                                var inputGroup = thisNode.append("g").attr("class","port_input");
                                inputGroup.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                                    .on("mousedown",function(d){portMouseDown(d,1,0);})
                                    .on("touchstart",function(d){portMouseDown(d,1,0);})
                            ...
                            } else if (d.inputs === 4 && inputPorts.empty()) {
                                var inputGroup = thisNode.append("g").attr("class","port_input");
                                inputGroup.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                                    .on("mousedown",function(d){portMouseDown(d,1,0);})
                                    .on("touchstart",function(d){portMouseDown(d,1,0);})
                                ...
                                inputGroup.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                                    ...
                                    .on("mouseout",function(d) { var port = d3.select(this); port.classed("port_hovered",false);})
                                if(Array.isArray(d._def.inputLabels)) {
                                  inputGroup.append("text").attr("stroke-width", "0px").attr("font-size", "12px").text(d._def.inputLabels[4]);
                                }

                            } else if (d.inputs === 6 && inputPorts.empty()) {
                                var inputGroup = thisNode.append("g").attr("class","port_input");
                                inputGroup.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                                    .on("mousedown",function(d){portMouseDown(d,1,0);})
                                    .on("touchstart",function(d){portMouseDown(d,1,0);})
                                ...
                                var inputGroup = thisNode.append("g").attr("class","port_input");
                                inputGroup.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                                    .on("mousedown",function(d){portMouseDown(d,1,5);})
                                    .on("touchstart",function(d){portMouseDown(d,1,5);})
                                    .on("mouseup",function(d){portMouseUp(d,1,5);} )
                                    .on("touchend",function(d){portMouseUp(d,1,5);} )
                                    .on("mouseover",function(d) { var port = d3.select(this); port.classed("port_hovered",(mouse_mode!=RED.state.JOINING || (drag_lines.length > 0 && drag_lines[0].portType !== 1) ));})
                                    .on("mouseout",function(d) { var port = d3.select(this); port.classed("port_hovered",false);})
                                if(Array.isArray(d._def.inputLabels)) {
                                  inputGroup.append("text").attr("stroke-width", "0px").attr("font-size", "12px").text(d._def.inputLabels[5]);
                                }

                            } else if (d.inputs === 7 && inputPorts.empty()) {
                                var inputGroup = thisNode.append("g").attr("class","port_input");
                                inputGroup.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                                    .on("mousedown",function(d){portMouseDown(d,1,0);})
                                    .on("touchstart",function(d){portMouseDown(d,1,0);})
                                ...
                                var inputGroup = thisNode.append("g").attr("class","port_input");
                                inputGroup.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                                    .on("mousedown",function(d){portMouseDown(d,1,6);})
                                    .on("touchstart",function(d){portMouseDown(d,1,6);})
                                    .on("mouseup",function(d){portMouseUp(d,1,6);} )
                                    .on("touchend",function(d){portMouseUp(d,1,6);} )
                                    .on("mouseover",function(d) { var port = d3.select(this); port.classed("port_hovered",(mouse_mode!=RED.state.JOINING || (drag_lines.length > 0 && drag_lines[0].portType !== 1) ));})
                                    .on("mouseout",function(d) { var port = d3.select(this); port.classed("port_hovered",false);})
                                if(Array.isArray(d._def.inputLabels)) {
                                  inputGroup.append("text").attr("stroke-width", "0px").attr("font-size", "12px").text(d._def.inputLabels[6]);
                                }

                            } else if (d.inputs === 8 && inputPorts.empty()) {
                                var inputGroup = thisNode.append("g").attr("class","port_input");
                                inputGroup.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                                    .on("mousedown",function(d){portMouseDown(d,1,0);})
                                    .on("touchstart",function(d){portMouseDown(d,1,0);})
                                ...
                                var inputGroup = thisNode.append("g").attr("class","port_input");
                                inputGroup.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                                    .on("mousedown",function(d){portMouseDown(d,1,7);})
                                    .on("touchstart",function(d){portMouseDown(d,1,7);})
                                    .on("mouseup",function(d){portMouseUp(d,1,7);} )
                                    .on("touchend",function(d){portMouseUp(d,1,7);} )
                                    .on("mouseover",function(d) { var port = d3.select(this); port.classed("port_hovered",(mouse_mode!=RED.state.JOINING || (drag_lines.length > 0 && drag_lines[0].portType !== 1) ));})
                                    .on("mouseout",function(d) { var port = d3.select(this); port.classed("port_hovered",false);})
                                if(Array.isArray(d._def.inputLabels)) {
                                  inputGroup.append("text").attr("stroke-width", "0px").attr("font-size", "12px").text(d._def.inputLabels[7]);
                                }
                            ...
                            // output_group.append("text").attr("stroke-width", "0px").attr("font-size", "12px").text(d._def.outputLabels[i]);
 //                            console.log("1320")
 //                            console.log(d._ports);
                            ...
                            if (d._ports) {
                                ...
                                d._ports.each(function(d,i) {
                                        ...
                                        // if(Array.isArray(d._def.outputLabels)) {
                                        //   console.log(d._def.outputLabels, i);
                                        //   output_group.append("text").attr("stroke-width", "0px").attr("font-size", "14px").text(d._def.outputLabels[i]);
                                        // }
                                });
                            }
...
//                             }
                            ...
                            thisNode.selectAll(".port_input").each(function(d,i) {
                                    var port = d3.select(this);
                                    port.attr("transform",function(d){return "translate(-5,"+((d.h/2)-5+(20*i))+")";})
                                    // console.log("hier");
                                    // console.log(d);
                                    // if(Array.isArray(d._def.inputLabels)) {
                                    //   port.append("text").attr("stroke-width", "0px").attr("font-size", "12px").text(d._def.inputLabels[i]);
                                    // }
                            });
            ...
            var link = vis.selectAll(".link").data(
                ...
                function(d) {
                  //console.log(d);
                    return d.source.id+":"+d.sourcePort+":"+d.target.id+":"+d.target.i+":"+d.targetPort;
                }
            );
            var linkEnter = link.enter().insert("g",".node").attr("class","link");

            linkEnter.each(function(d,i) {
              // console.log("adding link svg");
//               console.log(d);
//               console.log(i);
            ...
            links.each(function(d) {
                ...
                if (d.added || d===selected_link || d.selected || dirtyNodes[d.source.id] || dirtyNodes[d.target.id]) {
                    link.attr("d",function(d){
                        ...
                        d.y2 = d.target.y + (d.targetPort * 20);
